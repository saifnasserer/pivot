...
class Responsive {
  ...
  static double text(BuildContext context, {TextSize size = TextSize.medium}) {
    ...
    final multiplier = screenWidth >= 900 ? 1.5 : (screenWidth >= 600 ? 1.25 : 1.0);
  ...
  static EdgeInsets paddingHorizontal(BuildContext context, {Space size = Space.medium}) {
    return EdgeInsets.symmetric(horizontal: space(context, size: size));
  }

  // Vertical padding that automatically adapts to screen size
  static EdgeInsets paddingVertical(BuildContext context, {Space size = Space.medium}) {
