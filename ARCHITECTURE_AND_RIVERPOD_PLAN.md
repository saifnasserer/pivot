...
- `ProfileService` (Firestore/Storage) + `ProfileRepository`.
- `profileProvider` (`AsyncNotifier`, autoDispose) loads/updates profile, password, avatar via repository.
- Move profile-only widgets under `features/profile/ui/widgets`.
...
- `SubjectsService` + `SubjectsRepository`.
- `subjectsProvider` (`AsyncNotifier`, autoDispose) for CRUD, filtering, pagination via repository (wrap `PaginationService`).
...
- `AdminUsersService` (Firestore + Auth + Deletion) + `AdminUsersRepository`.
- `adminUsersProvider` (`AsyncNotifier`, autoDispose) for load/filter/update role/delete/bulk via repository.
- `AdminSectionsService`/`Repository` + `adminSectionsProvider`.
- `AdminGlobalSubjectsService`/`Repository` + `adminGlobalSubjectsProvider`.
- Split monolithic screens (e.g., user management) into smaller UI widgets.
...
- `NotificationsService` + `NotificationsRepository` + `notificationsProvider` (autoDispose where screen-scoped).
- `FeedbackService` + `FeedbackRepository` + `feedbackProvider`.
- Keep background services; expose actions via providers (no UI → service calls).
...
- `SuperAdminService`/`Repository` + `superAdminProvider`.
- `AnalyticsService`/`Repository` + `analyticsProvider`.

Acceptance:

- Panels load fast with loading/empty states; routes unchanged.
...
- `MediaService` (video/pdf) + `MediaRepository` + `mediaProvider`.
- `MaterialsService` + `MaterialsRepository` + `materialsProvider`.
...
- `TeamsService`/`Repository` + `teamsProvider`.
- `TasksService`/`Repository` + `tasksProvider` (filtering/bulk actions in notifier).
...
- `SettingsService`/`Repository` + `settingsProvider`.
- `BookmarksService`/`Repository` + `bookmarksProvider`.
- `ScheduleService`/`Repository` + `scheduleProvider`.
- `UserNotificationsService`/`Repository` + `userNotificationsProvider`.
